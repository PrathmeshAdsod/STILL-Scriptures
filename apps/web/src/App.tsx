...
import { timelinePercent } from './playback/momentTimeline';
...
const momentMarkerPreferenceKey = 'still:moment-markers';

function savedMomentMarkerPreference() {
  try { return localStorage.getItem(momentMarkerPreferenceKey) !== 'off'; }
  catch { return true; }
}
...
function Watch({ projectId }: { projectId: string }) {
  ...
  const [error, setError] = useState<string>();
  const [markersVisible, setMarkersVisible] = useState(savedMomentMarkerPreference);
  ...
  function toggleMomentMarkers() {
    setMarkersVisible((visible) => {
      const next = !visible;
      try { localStorage.setItem(momentMarkerPreferenceKey, next ? 'on' : 'off'); } catch { /* Preference persistence is optional. */ }
      return next;
    });
  }
  function revisitEcho(echo: Echo) {
    adapterRef.current?.pause();
    adapterRef.current?.seek(echo.knowledge_cutoff_seconds);
    setTime(echo.knowledge_cutoff_seconds);
    setExpandedEchoId(echo.id);
  }
