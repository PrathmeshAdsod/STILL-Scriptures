...
function libraryStatus(item: ProjectLibraryItem): { label: string; detail: string } {
  ...
  if (project.status === 'READY') {
    ...
    if (session?.contiguous_frontier_seconds) return { label: 'CONTINUE WATCHING', detail: `Saved at ${formatTime(session.contiguous_frontier_seconds)}.` };
...
function Watch({ projectId }: { projectId: string }) {
    ...
    void (async () => {
      try {
        ...
        if (active) {
          ...
          setResumeAt(started.contiguous_frontier_seconds);
          setTime(started.contiguous_frontier_seconds);
          setSession(started);
        }
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        if (resumeAt > 1 && source.duration_seconds && resumeAt < source.duration_seconds - 2) adapter.seek(resumeAt);
        const sync = async (endedNaturally = false) => {
          ...
          try {
            const next = await api.updateSession(projectId, sessionId, { ranges: trackerRef.snapshot(), duration_seconds: source.duration_seconds, ended_naturally: saveNaturalEnd });
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [projectId, session?.contiguous_frontier_seconds, sessionId]);
